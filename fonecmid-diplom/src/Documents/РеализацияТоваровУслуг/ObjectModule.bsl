...
Процедура ОбработкаПроведения(Отказ, Режим)
   ...
   // ++ Соколовская О.В. 24.09.2026 Отчет "Анализ выставленных актов"
   Движения.ВКМ_ОплатаРаботСпециалистов.Записывать = Истина;
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ
		...
		|ГДЕ
		|	РеализацияТоваровУслугУслуги.Номенклатура = &НоменклатураРаботыСпециалиста
	...
	Запрос.УстановитьПараметр("НоменклатураРаботыСпециалиста", Константы.ВКМ_НоменклатураРаботыСпециалиста.Получить());
	...
	Если Выборка.Следующий() Тогда
		Движение = Движения.ВКМ_ОплатаРаботСпециалистов.Добавить();
		ЗаполнитьЗначенияСвойств(Движение, Выборка);
	КонецЕсли;
   // -- Соколовская О.В. 24.09.2026
   
КонецПроцедуры

// ++ Соколовская О.В. 15.09.2026 Заполнение реализации товаров и услуг
Процедура ВыполнитьАвтозаполнение(СуммаАбонентскойПлаты) Экспорт
	
	Ошибка = Ложь;
	
	НоменклатураАбонентскаяПлата = Константы.ВКМ_НоменклатураАбонентскаяПлата.Получить();
	
	Если НоменклатураАбонентскаяПлата.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Не заполнено значение константы ""Номенклатура абонентская плата""'");
		Сообщение.Сообщить();
		Ошибка = Истина;
	КонецЕсли;
	...
	Если НоменклатураРаботыСпециалиста.Пустая() Тогда
		Сообщение = Новый СообщениеПользователю;
		Сообщение.Текст = НСтр("ru = 'Не заполнено значение константы ""Номенклатура работы специалиста""'");
		Сообщение.Сообщить();
		Ошибка = Истина;
	КонецЕсли;	
	...
	Если СуммаАбонентскойПлаты <> 0 Тогда
		...
		Стр.Количество = 1;
		Стр.Сумма = СуммаАбонентскойПлаты;
	КонецЕсли;
	...
	Если Выборка.Следующий() Тогда
		Стр = Услуги.Добавить();
		Стр.Номенклатура = НоменклатураРаботыСпециалиста;
		Стр.Количество = Выборка.КоличествоЧасовОборот;
		Стр.Сумма = Выборка.СуммаКОплатеОборот;
	КонецЕсли;
	
КонецПроцедуры  // -- Соколовская О.В. 15.09.2026
